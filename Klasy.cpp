...
BazaKsiazek::BazaKsiazek() {
	...
	if (!plik) {
		ofstream nowy_plik(nazwa_pliku); // Tworzymy plik, jeśli nie istnieje
	}
	else {
		loadFromFile();
	}
...
void BazaKsiazek::loadFromFile() {
	...
	while (std::getline(plik, linia)) {
		...
		// Rozdziel linie na poszczególne części, oddzielone przecinkami
...
void BazaKsiazek::saveToFile() {
	...
	if (!plik) {
		std::cerr << "Nie można otworzyć pliku do zapisu: " << nazwa_pliku << std::endl;
		return;
	}
...
// Funkcja pomocnicza do usuwania spacji z początku i końca stringa
...
void BazaKsiazek::add(string t, string au, int isbn, string okl) {
	if (wyszukaj_czy_jest(isbn)) {
		throw runtime_error("Książka o podanym ISBN już istnieje.");
	}
	else {
		if (wyszukaj_czy_jest(t)) {
			throw runtime_error("Książka o podanym tytule już istnieje.");
		}
		else {
			...
			this->loadFromFile(); // Aktualizacja listy książek
		}
	}
}
...
Ksiazka BazaKsiazek::wyszukaj(const std::string& tyt) const {
	...
	throw std::runtime_error("Nie znaleziono książki o podanym tytule.");
}
...
Ksiazka BazaKsiazek::wyszukaj(int isbn) const {
	...
	throw runtime_error("Nie znaleziono książki o podanym ISBN.");
}
...
void Bibliotekarz::akceptacja_wyp(BazaWypozyczen& baza_wyp, int isbn, BazaSkrytek& baza_skrytek, BazaUzytkownikow& baza_uz, BazaKsiazek& baza_ks) {
	...
	if (it != wypozyczenia.end()) {
		...
		string data_odd = addDaysToDate(data_wyp, 30);
		...
		std::cout << "Wypożyczenie zaakceptowane: ISBN " << isbn << " w skrytce " << numer_skrytki << "." << std::endl;
	}
	else {
		throw std::runtime_error("Nie znaleziono wypożyczenia o podanym ISBN.");
	}
}
...
void Bibliotekarz::sendEmail(string email, string login, string tyt, string dat, int num) {
	...
	if (plik) {
		...
		cout<<"Nie udało się wysłać emaila"<<endl;
	}
}
...
void BazaUzytkownikow::add(string i, string n, string e, string a, int tel, string l, string h) {
	...
	ofstream plik(nazwa_pliku, ios::app);
	if (!plik) {
		cerr << "Błąd przy otwieraniu pliku: " << nazwa_pliku << endl;
	}
	else {
	...
	cout << "Dodano użytkownika: " << nowy.imie << " " << nowy.nazwisko << endl;

	loadFromFile(); // Aktualizacja danych w pamięci
}
...
void BazaUzytkownikow::usun(const std::string& login) {
	...
	if (it != lista_czytelnikow.end()) {
		...
		throw std::runtime_error("Nie znaleziono użytkownika");
	}
}
...
Czytelnik BazaUzytkownikow::wyszukaj(const string& login) const {
	...
	throw runtime_error("Nie znaleziono użytkownika o podanym loginie.");
}
...
ostream& operator<<(ostream& os, const Wypozyczenie& w) {
	os << "Data wypożyczenia: " << w.data_wypozyczenia << "\nData oddania: " << w.data_oddania << "\nNumer skrytki: " << w.numer_skrytki << "\nISBN książki: " << w.isbn_ksiazki << "\nLogin użytkownika: "<<w.login_czytelnika<<"\n";
	return os;
}
...
void Wypozyczenie::prolong(int num, string data) {
	data_oddania = replaceDay(data, num);
	data_oddania += "*";
}
...
BazaSkrytek::BazaSkrytek(int num) {
	...
	if (!plik.is_open() || plik.peek() == std::ifstream::traits_type::eof()) {
		// Jeśli plik nie istnieje lub jest pusty, tworzymy nową listę skrytek
		lista_skrytek.clear(); // Upewniamy się, że lista jest pusta
		for (int i = 0; i < num; i++) {
			int id = i + 1;
			bool wolna = true;
		...
		saveToFile(); // Zapisujemy nowo utworzoną listę do pliku
	}
	else {
		loadFromFile(); // Wczytujemy dane z istniejącego pliku
		plik.close();
	}
}
...
void BazaSkrytek::add() {
	...
	saveToFile(); // Zapisujemy aktualizowaną listę do pliku
}


void BazaSkrytek::zajmij(int id, int isbn) {
	if (id > 0 && id <= static_cast<int>(lista_skrytek.size())) {
		Skrytka& skrytka = lista_skrytek[id - 1]; // Indeksy zaczynają się od 0

		if (!skrytka.wolna) {
			std::cerr << "Skrytka " << id << " jest już zajęta!" << std::endl;
			return;
		}
		...
		std::cout << "Skrytka " << id << " została zajęta dla ISBN " << isbn << "." << std::endl;
	}
	else {
		std::cerr << "Nieprawidłowy numer skrytki: " << id << std::endl;
	}
}
...
void BazaSkrytek::zwolnij(int id) {
	if (id > 0 && id <= static_cast<int>(lista_skrytek.size())) {
		...
		if (!skrytka.wolna) {
			skrytka.wolna = true;
			skrytka.numer_wypozyczenia = -1; // Ustaw wartość domyślną
			saveToFile(); // Zapisz zmiany do pliku
			std::cout << "Skrytka " << id << " została zwolniona.\n";
		}
		else {
			std::cout << "Skrytka " << id << " już jest wolna.\n";
		}
	}
	else {
		std::cerr << "Nieprawidłowy numer skrytki: " << id << "\n";
	}
}
...
void BazaSkrytek::usun_skrytke(int id) {
	...
	throw runtime_error("Nie znaleziono skrytki o podanym ID do usunięcia.");
}

int BazaSkrytek::getFirstFree() {
	for (size_t i = 0; i < lista_skrytek.size(); ++i) {
		if (lista_skrytek[i].wolna) {
			return lista_skrytek[i].id; // Zwróć ID skrytki, a nie indeks
		}
	}
...
ostream& operator<<(ostream& os, const Skrytka& skrytka) {
	os << "[" << skrytka.id << ", wolna: " << (skrytka.wolna ? "tak" : "nie")
		<< ", numer wypożyczenia: " << skrytka.numer_wypozyczenia << "]\n";
	return os;
}
...
void BazaWypozyczen::loadFromFile_pocz() {
	...
	if (!plik.is_open()) {
		std::cerr << "Nie można otworzyć pliku: " << nazwa_pliku_pocz << std::endl;
		return;
	}
...
void BazaWypozyczen::end_wyp(int isbn) {
	// Znajdź i usuń wypożyczenie z poczekalni
	...
	if (it != lista_wypozyczen_pocz.end()) {
		...
		std::cout << "Usunięto wypożyczenie dla ISBN: " << isbn << std::endl;
	}
	else {
		std::cerr << "Nie znaleziono wypożyczenia dla ISBN: " << isbn << std::endl;
	}
}
...
void BazaWypozyczen::saveToFile_pocz() {
	...
	if (!plik.is_open()) {
		std::cerr << "Nie można otworzyć pliku do zapisu: " << nazwa_pliku_pocz << std::endl;
		return;
	}
...
void BazaWypozyczen::przyznanieWyp(string login, int isbn, BazaSkrytek baza_skrytek) {
	if (isBookBorrowed(isbn)) {
		cout << "Książka o ISBN " << isbn << " jest już wypożyczona.\n";
		return;
	}
	...
	// Dodanie wypożyczenia do poczekalni
...
std::vector<Wypozyczenie> BazaWypozyczen::getlista_wypozyczen_pocz() {
	...
	return lista_wypozyczen_pocz; // Zwróć wektor
}
...
bool BazaWypozyczen::isBookBorrowed(int isbn) const {
	// Sprawdzenie w głównej liście wypożyczeń
	for (const auto& wypozyczenie : lista_wypozyczen) {
...
void BazaWypozyczen::przeniesDoZwrotow(int isbn) {
	...
	if (it != lista_wypozyczen.end()) {
		// Dodaj wypożyczenie do listy zwrotów
		lista_wypozyczen_zwroty.push_back(*it);

		// Usuń wypożyczenie z listy aktywnych
		...
		std::cerr << "Nie znaleziono wypożyczenia o ISBN: " << isbn << " w aktywnych wypożyczeniach.\n";
	}
}

void BazaWypozyczen::loadFromFileZwroty() {
	lista_wypozyczen_zwroty.clear(); // Używamy tej samej listy, co dla poczekalni zwrotów
	std::ifstream plik(nazwa_pliku_zwrotow);

	if (!plik.is_open()) {
		std::cerr << "Nie można otworzyć pliku: " << nazwa_pliku_zwrotow << std::endl;
		return;
	}
...
void BazaWypozyczen::saveToFileZwroty() {
	...
	if (!plik.is_open()) {
		std::cerr << "Nie można otworzyć pliku do zapisu: " << nazwa_pliku_zwrotow << std::endl;
		return;
	}
...
std::vector<Wypozyczenie> BazaWypozyczen::getlista_wypozyczen_zwroty() {
	...
	return lista_wypozyczen_zwroty; // Zwróć wektor
}
...
void BazaWypozyczen::usunZwrot(int isbn) {
	...
	if (it != lista_wypozyczen_zwroty.end()) {
		lista_wypozyczen_zwroty.erase(it);  // Usuń zwrot z wektora
...
void BazaWypozyczen::checkAndBlockLateUsers(BazaUzytkownikow& baza_uzytkownikow) {
	for (auto wyp : lista_wypozyczen) {
		if (isPastDate(wyp.data_oddania)) { // Sprawdź, czy data oddania minęła
			std::cerr << "Opóźnione zwrócenie książki przez użytkownika: "
